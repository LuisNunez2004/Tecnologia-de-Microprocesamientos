...
#define BOTON_TARJETA1 PB0
#define BOTON_TARJETA2 PB1

void mfrc522_standard_2(uint8_t *uid)
{
	static uint8_t initialized = 0;

	// Configuración inicial de pines
	if (!initialized) {
		DDRB &= ~((1 << BOTON_TARJETA1) | (1 << BOTON_TARJETA2));
		PORTB |= (1 << BOTON_TARJETA1) | (1 << BOTON_TARJETA2);
		initialized = 1;
	}

	static uint8_t tarjeta1[5] = {0xDE, 0xAD, 0xBE, 0xEF, 0x01};
	static uint8_t tarjeta2[5] = {0x01, 0x23, 0x45, 0x67, 0x89};

	// Por defecto no hay tarjeta
	for (uint8_t i = 0; i < 5; i++) uid[i] = 0;

	// Leer botones
	if (!(PINB & (1 << BOTON_TARJETA1))) {
		for (uint8_t i = 0; i < 5; i++) uid[i] = tarjeta1[i];
		} else if (!(PINB & (1 << BOTON_TARJETA2))) {
		for (uint8_t i = 0; i < 5; i++) uid[i] = tarjeta2[i];
	}
}
